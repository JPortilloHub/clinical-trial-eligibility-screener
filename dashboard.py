...
<style>
    ...
    /* Buttons - unified styling for export buttons */
    ...
    /* Email button - same style as download buttons */
    .email-btn {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        gap: 0.5rem;
        background: linear-gradient(135deg, #0f4c81 0%, #1a6eb0 100%);
        color: white;
...
with st.sidebar:
    ...
    <div style="text-align: center; padding: 1rem 0 1.5rem 0;">
        ...
        <div style="font-size: 1.1rem; font-weight: 700; color: #0f4c81; margin-top: 0.5rem;">
            Trial Screener
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if results:
        st.markdown('<p class="filter-title">Filter Results</p>', unsafe_allow_html=True)

        # Status filter with better UX
        st.markdown("**Eligibility Status**")
        col1, col2 = st.columns(2)
        with col1:
            show_eligible = st.checkbox("Eligible", value=True, key="f_eligible")
            show_likely = st.checkbox("Likely Eligible", value=True, key="f_likely")
        with col2:
            show_not_eligible = st.checkbox("Not Eligible", value=True, key="f_not_eligible")
            show_unclear = st.checkbox("Unclear", value=True, key="f_unclear")

        # Build filter list
        filter_status = []
        if show_eligible:
            filter_status.append("ELIGIBLE")
        if show_not_eligible:
            filter_status.append("NOT_ELIGIBLE")
        if show_likely:
            filter_status.append("LIKELY_ELIGIBLE")
        if show_unclear:
            filter_status.append("UNCLEAR")
        ...
        if filter_status:
            filtered_results = [
                r for r in results
                if r.get("overall_eligibility") in filter_status
        ...
        st.caption(f"Showing {len(filtered_results)} of {len(results)} patients")
    ...
    st.markdown("**About**")
    st.caption("""
    AI-powered patient screening for clinical trial eligibility.
    Results are generated by analyzing patient records against trial criteria.
    """)
...
if not results:
    ...
    st.warning("No patients match the current filter criteria. Please adjust the filters in the sidebar.")

    # Still show the metrics for all data
    total_all = len(results)
    eligible_all = sum(1 for r in results if r.get("overall_eligibility") == "ELIGIBLE")
    not_eligible_all = sum(1 for r in results if r.get("overall_eligibility") == "NOT_ELIGIBLE")
    review_all = total_all - eligible_all - not_eligible_all

    st.markdown("### All Patients Summary (Unfiltered)")
    col1, col2, col3 = st.columns(3)
    col1.metric("Total", total_all)
    col2.metric("Eligible", eligible_all)
    col3.metric("Requires Review", review_all)
    ...
    with tab1:
        # Add spacing between tabs and KPIs
    ...
    with tab2:
        st.markdown('<p class="section-header">Individual Patient Assessment</p>', unsafe_allow_html=True)

        # Patient selector
        patient_ids = [r.get("patient_id", f"Patient {i}") for i, r in enumerate(filtered_results)]

        selected_patient = st.selectbox(
            "Select a patient to view detailed assessment",
            patient_ids,
            key="patient_select",
        ...
        selected_data = next((r for r in filtered_results if r.get("patient_id") == selected_patient), filtered_results[0])
    ...
    with tab3:
        ...
        with action_col1:
            # Excel export
            excel_data = create_excel_report(filtered_results)
            st.download_button(
        ...
        with action_col3:
            ...
            email_body = urllib.parse.quote(generate_email_body(filtered_results))
            email_link = f"mailto:?subject={email_subject}&body={email_body}"

            st.markdown(f"""
            <a href="{email_link}" class="email-btn">
                ✉️ Send via Email
            </a>
            """, unsafe_allow_html=True)
